...
address='0x07f3d316630719f4fc69c152f397c150f0831071' # eurt usdt
...
md.loc[md.pct==.001].plot('date','marketdepth')
...
#
#
# dfm2.loc[block_number==].amount.sum()/dfm2.loc[block_number==].amount.count()
# dfm['amount']=L
#
#
# tmplist=list()
# for pc in tqdm(pctchg):
#     tmp=dpu2.genMarketDepth(dfm,delta=pc,ts=ts, plusminus=False,decimals0=decimals0,logdelta=False)
#     tmp['pct']=pc
#     tmplist.append(tmp)
#
...
#
#
# x=liqsumbydate.iloc[-1]['x']
# y=liqsumbydate.iloc[-1]['y']
# liqX=liqsumbydate.iloc[-1]['liqX']
# price=liqsumbydate.iloc[-1]['price']
# p=liqsumbydate.iloc[-1]['p']
# currenttick=liqsumbydate.iloc[-1]['currenttick']
#
#
#
#
...
#     p0=y2/x2
#
# def calcV2DepthfromTDL(total_deposit_locked,p,delta=np.round(np.arange(-.1,.1,.01)/.01)*.01):
#     # calculate v2 depth from TDL and price (y/x)
#     x=total_deposit_locked/2
#     L=x*np.sqrt(p)
#     return calcV2Depth(L,p0=p,delta=delta)
#
# def calcV2Depth(L,p0,delta=np.round(np.arange(-.1,.1,.01)/.01)*.01):
...
# liqsumbydate.head(1)
#
# i,ll = liqsumbydate.iterrows()[0]
# tmp=list()
# for i,ll in liqsumbydate.iterrows():
#     tmpdf=calcv2depth(x=ll.x, y=ll.y, price=ll.price)
#     tmpdf['date']=i[0]
...
#
# dc.set_index('date_x')[['marketdepth','v2depth']].plot()
#
#
#
# a=calcv2depth(x,y,price)
#
# L
# p0
# L/np.sqrt(p0*(0.1))-x2
#
#
# sns.lineplot(delta,a)
#
# from tqdm import tqdm
#
# dfmsingle=dfm.loc[dfm.block_number==14258800]
#
# dfmsingle
# pctchg=delta
# tmplist=list()
