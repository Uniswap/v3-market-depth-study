...
def genLiqRange(dft, tickwindow=60):
    ...
    if not (
        all(dft.tickLower % tickwindow == 0) & all(dft.tickUpper % tickwindow == 0)
    ):
    ...
    for i, row in dft.iterrows():
        dfrgn.loc[np.arange(row.tickLower, row.tickUpper, tickwindow), "amount"] = (
            dfrgn.loc[np.arange(row.tickLower, row.tickUpper, tickwindow), "amount"]
            + row["amount"]
...
def calcdollarliq(dfrgn, tickwindow=60, alt=0):
    ...
    dfrgn.loc[dfrgn.p <= dfrgn.pa, "x"] = (
        dfrgn.amount
        * (dfrgn.pb ** 0.5 - dfrgn.pa ** 0.5)
        / ((dfrgn.pa * dfrgn.pb) ** 0.5)
        / 1e6
    )
    dfrgn.loc[(dfrgn.pa < dfrgn.p) & (dfrgn.p < dfrgn.pb), "x"] = (
        dfrgn.amount
        * (dfrgn.pb ** 0.5 - dfrgn.p ** 0.5)
        / ((dfrgn.p * dfrgn.pb) ** 0.5)
        / 1e6
    )
    dfrgn.loc[dfrgn.p >= dfrgn.pb, "x"] = 0

    dfrgn.loc[dfrgn.p <= dfrgn.pa, "y"] = 0
    dfrgn.loc[(dfrgn.pa < dfrgn.p) & (dfrgn.p < dfrgn.pb), "y"] = (
        dfrgn.amount * (dfrgn.p ** 0.5 - dfrgn.pa ** 0.5) / 1e18
    )
    dfrgn.loc[dfrgn.p >= dfrgn.pb, "y"] = (
        dfrgn.amount * (dfrgn.pb ** 0.5 - dfrgn.pa ** 0.5) / 1e18
    )
...
def genFullLiqDistribution(dfrgns, tickwindow=60):
    ...
    for dt in dfrgns["date"].unique():
        liqdist.append(
            calc_usdliq_depth(dfrgns.loc[dfrgns.date == dt], tickwindow=tickwindow)
        )
...
def expandliqdistrib(df, depthpct=0.02, returnDepth=1, tickwindow=60):
    ...
    if depthpct == 1:
        ...
        tick_u = (
            int(np.log((1 / P_u * 1e12)) / np.log(1.0001) / tickwindow) + 1
        ) * tickwindow
        tick_l = (
            int(np.log((1 / P_l * 1e12)) / np.log(1.0001) / tickwindow) - 1
        ) * tickwindow
    ...
    for i, row in dft[
        (
            (dft.tickLower >= dft.tick_l - tickwindow)
            & (dft.tickLower <= dft.tick_u + tickwindow)
        )
    ].iterrows():
        dfrgn.loc[
            (dfrgn.tickLower >= row.tickLower)
            & (dfrgn.tickLower <= row.tickLower + tickwindow),
            "depthattick",
        ] = row["liqusdpertick"]
